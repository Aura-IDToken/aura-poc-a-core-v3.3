...
2. **Cryptographic Proofs**: Merkle root + leaf hashes + HMAC-SHA256 signatures (ETC)
...
6. **Determinism Report**: `determinism-report.json` generated by CI on every build (x86_64 + ARM64)
7. **Audit Layer Specification**: `docs/specs/AUDIT_LAYER_SPEC.md` — normative frozen specification
...
- HMAC-SHA256 certificate signing (current implementation)
- Certificate fingerprinting (unique + reproducible)

**Signing Architecture**:  
The Audit Layer uses an abstract `Signer`/`Verifier` interface.
The current implementation is HMAC-SHA256.  Future migration to
asymmetric signing (e.g., Ed25519) is supported through the abstraction
without changing the API or certificate schema.
...
6. Audit Layer Specification: `docs/specs/AUDIT_LAYER_SPEC.md` v1.0.0
